...
pub fn type_check_pgm(
    ...
    expr_tys: &mut SecondaryMap<ExprIdx, Type>,
) -> Result<(), TypeErr> {
    ...
    let ty = type_check(ctx, &mut ty_env, &mut subst_env, &mut scope, expr, arena)?;
...
fn type_check(
    ...
    expr: ExprIdx, arena: &mut ExprArena,
) -> Result<Type, TypeErr> {
    todo!()
    /*
        let mut expr = &mut arena[expr];
        match expr.kind {
